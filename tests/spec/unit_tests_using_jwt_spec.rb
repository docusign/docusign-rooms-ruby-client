...
describe 'DocuSign Ruby Client Tests' do
	def login
		begin
			if $api_client.nil?
				...
				$api_client.set_oauth_base_path(DocuSign_Rooms::OAuth::DEMO_OAUTH_BASE_PATH)
  ...
  before(:all) do
    ...
    $host = "https://demo.rooms.docusign.com/restapi"
	...
	$scopes = ["dtr.company.write", "dtr.company.read", "dtr.rooms.write", "dtr.rooms.read", "signature",  "impersonation"]
  end
  ...
  describe DocuSign_Rooms::OAuth do
  	describe '.login' do
  ...
  describe DocuSign_Rooms::RoomsApi do
  	describe '.get' do
  		context 'rooms' do
  			it 'successfully returns rooms' do
				api_client = create_api_client()
				rooms_api = DocuSign_Rooms::RoomsApi.new(api_client)

				options = DocuSign_Rooms::GetRoomsOptions.new

				# account_id, options = DocuSign_Rooms::GetRoomsOptions.default
				room_summary_list = rooms_api.get_rooms($account_id, options)

				expect(room_summary_list).to be_truthy
				if(!room_summary_list.nil?)
					expect(room_summary_list.result_set_size > 0).to be_truthy
				end
  			end
  		end
  	end
